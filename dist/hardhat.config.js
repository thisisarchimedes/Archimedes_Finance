...
dotenv_1.default.config({ path: (0, path_1.resolve)(__dirname, "./user.env") });
...
(0, config_1.task)("test:watch", "For hardhat watch to run tests on save for both test and sol files", async (taskArgs, hre) => {
...
(0, config_1.task)("test:log", "Run tests with all logger logs", async (taskArgs, hre) => {
    const { file } = taskArgs;
    (0, logger_1.setLoggingEnabled)(true);
    const args = file ? { testFiles: [file] } : undefined;
    await hre.run("test", args);
}).addOptionalParam("file", "If provided run tests only on specified file", undefined, config_1.types.inputFile);
