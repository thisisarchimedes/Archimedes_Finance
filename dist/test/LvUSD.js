...
describe("LvUSD contract test suit", function () {
    ...
    describe("Minting", function () {
        it("Should be able to mint to specific address", async function () {
            ...
            (0, chai_1.expect)(await r.lvUSD.totalSupply()).to.equal(ContractTestContext_1.ownerStartingLvUSDAmount.add(tokenSupply));
        });
    });
    ...
    it("Transaction should revert if sender doesn’t have enough tokens", async function () {
        ...
        await (0, chai_1.expect)(r.lvUSD.connect(r.addr1).transfer(r.owner.address, 1)).to.be.revertedWith("ERC20: transfer amount exceeds balance");
    });
    it("Balance should remain unchanged if transfer is reverted due to insufficient balance", async function () {
        ...
        (0, chai_1.expect)(await r.lvUSD.balanceOf(r.owner.address)).to.equal(initialOwnerBalance);
    });
