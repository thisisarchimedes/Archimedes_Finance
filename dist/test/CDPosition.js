...
describe("CDPosition test suit", async function () {
    const getFloatFromBigNumNoDecimal = (bigNumber) => {
        return parseFloat(hardhat_1.ethers.utils.formatUnits(bigNumber, 0));
    };
    async function validateCDP(nftID, principle, interestEarned, total, borrowed, shares) {
        (0, chai_1.expect)(await cdp.getOUSDPrinciple(nftID)).to.equal(principle);
        ...
        (0, chai_1.expect)(await cdp.getOUSDTotalWithoutInterest(nftID)).to.equal(total);
        (0, chai_1.expect)(await cdp.getLvUSDBorrowed(nftID)).to.equal(borrowed);
        (0, chai_1.expect)(await cdp.getShares(nftID)).to.equal(shares);
        (0, chai_1.expect)(getFloatFromBigNumNoDecimal(await cdp.getPositionTimeOpened(nftID))).to.closeTo(1657322079, 100000);
        (0, chai_1.expect)(await cdp.getPositionTimeToLive(nftID)).to.equal(hardhat_1.ethers.utils.parseUnits("369", 0));
        (0, chai_1.expect)(getFloatFromBigNumNoDecimal(await cdp.getPositionExpireTime(nftID))).to.closeTo(1689203679, 100000);
        // console.log(
    ...
    beforeEach(async () => {
        const r = await (0, ContractTestContext_1.buildContractTestContext)();
    ...
    describe("Create position", () => {
        ...
        it("Should not create position on existing NFT ID", async function () {
            ...
            await (0, chai_1.expect)(cdp.createPosition(NFT_ID, BASIC_OUSD_PRINCIPLE)).to.be.revertedWith("NFT ID must not exist");
        });
    });
    describe("Delete Position", function () {
        ...
        it("Should not delete position if position still has borrowed lvUSD", async function () {
            ...
            await (0, chai_1.expect)(cdp.deletePosition(NFT_ID)).to.be.revertedWith("lvUSD borrowed must be zero");
        });
        it("Should not delete position if NFT ID does not exist in mapping", async function () {
            await (0, chai_1.expect)(cdp.deletePosition(NFT_ID)).to.be.revertedWith("NFT ID must exist");
        });
    });
    describe("Add and remove shares from position", () => {
        ...
        it("Should revert when removing more shares than the position owns", async function () {
            await (0, chai_1.expect)(cdp.removeSharesFromPosition(NFT_ID, 20)).to.be.revertedWith("Shares exceed position balance");
        });
    });
    describe("Borrow and deposit actions for position", function () {
        ...
        it("Should revert if total deposited OUSD is lower then amount to withdraw", async function () {
            await (0, chai_1.expect)(cdp.withdrawOUSDFromPosition(NFT_ID, getEighteenDecimal(1100000))).to.be.revertedWith("Insufficient OUSD balance");
        });
    });
    describe("Changes to a specific NFT ID CDP struct should not effect other NFT IDs struct", function () {
        ...
        it("Should not alter any NFT ID CDP if it tried to alter a non existing NFT", async function () {
            await (0, chai_1.expect)(cdp.depositOUSDtoPosition(2349201840, amountInOUSDToDeposit)).to.be.revertedWith("NFT ID must exist");
