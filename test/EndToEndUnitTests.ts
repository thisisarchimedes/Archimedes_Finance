...
async function approveAndGetLeverageAsUser (
    _principleOUSD: BigNumber, _numberOfCycles:number, _archTokenAmount : BigNumber, _r: ContractTestContext, _user: SignerWithAddress,
) {
    logger("Will deposit %s OUSD principle that cost %s ArchToken for %s cycles", getFloatFromBigNum(_principleOUSD), getFloatFromBigNum(_archTokenAmount), _numberOfCycles);
...
//     it("Should be able to trasnfer admin role back to owner", async function () {
...
//     it("Should not not be able to change arch lev ratio as non Arch Govornor ", async function () {
...
//     it("Should be able to change Arch lev Ration as Arch Goveronor", async function () {
...
//     it("Should not allow regualt Gov to change ArchToLevRatio", async function () {
...
//         // Make user Gurdian, this is to make sure that Admin can't pause
...
//         const archCostOfLeverageIn18Dec = (await r.parameterStore.calculateArchNeededForLeverage(leverageUserIsTakingIn18Dec)).add(parseUnits("0.001"));
...
//     it("Should have loged all 5 positions in array", async function () {
...
//         await safeTransferAsUser(
//             user.address,
//             userOther.address,
//             1,
//         );

//         await safeTransferAsUserOther(
//             userOther.address,
//             user.address,
//             3,
//         );
...
describe("Test suit for getting leverage", function () {
    ...
    it("Should have experation related information for position", async function ()  {
        ...
        console.log("\x1B[31mPosition Opened Time %s, TTL %s, ExpireTime %s, day is %s, today is %s", positionTimeOpenedDate, positionTimeToLiveDate, positionExpireTimeDate, positionTimeOpenedDay, nowDate);
    ...
    it("Should have deposited leverage and principle in vault (minus fee)", async function () {
        ...
        console.log("\x1B[31mSimplePositionCreation: %s OUSD assets deposited in vault, expecting %s +- expectedLossDueToBorrowingFromPools (%s) ", vaultOUSDBalance, expectedVaultOUSDBalance ,expectedLossDueToBorrowingFromPools);
    ...
    it("Should increase funds in vault when an OUSD rebase happens", async function () {
        ...
        // actual test
        console.log("\x1B[31mSimplePositionCreation: Created a rebase of %s OUSD with %s rebaseFee. Total OUSD assets deposited in vault are %s while before it was %s. Delta is %s",
            rebaseAmount, rebaseRateFee, ousdInVaultAfterRebase, vaultAssetsBeforeRebase, ousdInVaultAfterRebase - vaultAssetsBeforeRebase);
    ...
    it("Should trasnfer OUSD funds (colletral + interest) to user", async function() {
        ...
        const expectedUserGainsFromUnwinding = contractEstimatedReturnedOUSDMinusInterestFromUnwinding + getFloatFromBigNum(cdpInterestEarnedIn18Dec) - 0.005 * userOUSDPrinciple  ; // 2ed to last bit is originationFee, last bit is accepted curve loss 
        console.log("\x1B[31mSimplePositionCreation: expecting ousd delta of %s for user's address while got %s. Our contract method to estimate unwind OUSD windfall gave %s (minus interest)", expectedUserGainsFromUnwinding,
            userActualGainsFromUnwinding, contractEstimatedReturnedOUSDMinusInterestFromUnwinding)
