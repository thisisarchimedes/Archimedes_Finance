...
describe("Coordinator Test suit", function () {
    ...
    describe("Deposit collateral into new NFT position", function () {
        ...
        it("Should have increased vault balance on OUSD", async function () {
            const balance = await r.externalOUSD.balanceOf(r.vaultOUSD.address);
            expect(balance).to.equal(addr1CollateralAmount);
        });
        it("Should have increased OUSD in the vault", async function () {
            expect(await r.vaultOUSD.totalAssets()).to.equal(addr1CollateralAmount);
        });
        it("Should have increased OUSD shares", async function () {
            expect(await r.vaultOUSD.totalSupply()).to.equal(addr1CollateralAmount);
        });
        it("Should have given shares to shares owner", async function () {
            expect(await r.vaultOUSD.maxRedeem(sharesOwnerAddress)).to.equal(addr1CollateralAmount);
        });
        it("Should create entry for first depositer principle in CDP", async function () {
            expect(await r.cDPosition.getOUSDPrinciple(nftIdAddr1Position)).to.equal(addr1CollateralAmount);
        });
        it("Should create entry for first depositer shares in CDP", async function () {
            expect(await r.cDPosition.getShares(nftIdAddr1Position)).to.equal(addr1CollateralAmount);
        });
        ...
        describe("Separate deposits made by another user", function () {
            ...
            it("Should have increased OUSD in the vault by second collateral amount", async function () {
                expect(await r.vaultOUSD.totalAssets()).to.equal(combinedCollateralAmount);
            });
            it("Should have increased OUSD shares by second collateral amount", async function () {
                expect(await r.vaultOUSD.totalSupply()).to.equal(combinedCollateralAmount);
            });
            it("Should have increased shares to shares owner by second collateral amount", async function () {
                expect(await r.vaultOUSD.maxRedeem(sharesOwnerAddress)).to.equal(combinedCollateralAmount);
            });
            it("Should create entry for second depositer principle in CDP", async function () {
                expect(await r.cDPosition.getOUSDPrinciple(nftIdAddr2Position)).to.equal(addr2CollateralAmount);
            });
            it("Should create entry for second depositer shares in CDP", async function () {
                expect(await r.cDPosition.getShares(nftIdAddr2Position)).to.equal(addr2CollateralAmount);
            });
        ...
        describe("Borrow lvUSD for position", function () {
            ...
            before(async function () {
                ...
                await r.lvUSDToken.mint(coordinator.address, ethers.utils.parseUnits("100"));
                // method under test
            ...
            it("Should transfer lvUSD to exchanger address", async function () {
                ...
                expect(await r.lvUSDToken.balanceOf(r.exchanger.address)).to.equal(lvUSDAmountToBorrow);
            });
            it("Should decrease coordinator lvUSD balance", async function () {
                ...
                expect(await r.lvUSDToken.balanceOf(coordinator.address)).to.equal(ethers.utils.parseUnits("98"));
            });
            it("Should update CDP with borrowed lvUSD", async function () {
                expect(await r.cDPosition.getLvUSDBorrowed(nftIdFirstPosition)).to.equal(lvUSDAmountToBorrow);
            });
            ...
            describe("Repay lvUSD for position", function () {
                ...
                //     expect(getFloatFromBigNum(await r.lvUSDToken.balanceOf(coordinator.address)))
                ...
                //     expect(await r.lvUSDToken.balanceOf(r.exchanger.address)).to.equal(lvUSDAmountToRepayInTwoParts);
                //     expect(getFloatFromBigNum(await r.lvUSDToken.balanceOf(r.exchanger.address)))
                //         .to.closeTo(getFloatFromBigNum(lvUSDAmountToRepayInTwoParts), 0.2);
                // });
                it("Should update CDP with repayed lvUSD", async function () {
                    expect(getFloatFromBigNum(await r.cDPosition.getLvUSDBorrowed(nftIdFirstPosition)))
                        .to.closeTo(getFloatFromBigNum(lvUSDAmountToRepayInTwoParts), 0.2);
                });
            });
            ...
            describe("Get leveraged OUSD for position", function () {
                ...
                before(async function () {
                    /// Get initial state
                    borrowedLvUSDInPositionBeforeLeverage = await r.cDPosition.getLvUSDBorrowed(nftIdFirstPosition);
                    ...
                    // method under test
                    depositedOUSDBeforeLeverage = await r.vaultOUSD.totalAssets();
                    originationFee = await r.parameterStore.calculateOriginationFee(leverageAmount);
                    sharesTotalSupplyBeforeLeverage = await r.vaultOUSD.maxRedeem(sharesOwnerAddress);
                ...
                it("Should have increase borrowed amount on CDP for NFT", async function () {
                    expect(await r.cDPosition.getLvUSDBorrowed(nftIdFirstPosition)).to.equal(
                ...
                it("Should have increased OUSD deposited in vault", async function () {
                    ...
                    // expect(await r.vaultOUSD.totalAssets()).to.equal(leverageAmount.add(depositedOUSDBeforeLeverage).sub(originationFee));
                    expect(getFloatFromBigNum(await r.vaultOUSD.totalAssets())).to.closeTo(expectedTotalAssets, 10);
                });
                it("Should have minted (more) shares to owner address", async function () {
                    expect(await r.vaultOUSD.maxRedeem(sharesOwnerAddress)).to.gt(
                ...
                it("Should have increased deposited (or totalOUSD) OUSD in CDPosition", async function () {
                    ...
                    expect(getFloatFromBigNum(await r.cDPosition.getOUSDTotal(nftIdFirstPosition)))
                ...
                it("Should have update CDPosition with shares", async function () {
                    ...
                    expect(getFloatFromBigNum(await r.cDPosition.getShares(nftIdFirstPosition)))
                ...
                describe("Unwind leveraged position", function () {
                    ...
                    before(async function () {
                        vaultOUSDAmountBeforeUnwind = await r.vaultOUSD.totalAssets();
                        positionTotalOUSD = await r.cDPosition.getOUSDTotal(nftIdFirstPosition);
                        positionShares = await r.cDPosition.getShares(nftIdFirstPosition);
                        positionExpectedOUSDTotalPlusInterest = await r.vaultOUSD.convertToAssets(positionShares);
                    ...
                    it(`Should reduce assets in Vault by the entire OUSD amount of
                        ...
                        expect(await r.vaultOUSD.totalAssets()).to.equal(
                    ...
                    it("Should have deleted CDP position", async function () {
                        ...
                        try {
                            await r.cDPosition.getOUSDPrinciple(nftIdFirstPosition);
    ...
    describe("Coordinator overview testing", function () {
        ...
        before(async function () {
            ...
            await r.lvUSDToken.mint(r.coordinator.address, mintedLvUSDAmount);
        ...
        it("Should have updated CDP getOUSDPrinciple with values for leveraged position", async function () {
            expect(await r.cDPosition.getOUSDPrinciple(endToEndTestNFTId)).to.equal(collateralAmount);
        });

        it("Should have updated CDP getOUSDInterestEarned with values for leveraged position", async function () {
            expect(await r.cDPosition.getOUSDInterestEarned(endToEndTestNFTId)).to.equal(0);
        });

        it("Should have updated CDP getOUSDTotal with values for leveraged position", async function () {
            expect(getFloatFromBigNum(await r.cDPosition.getOUSDTotal(endToEndTestNFTId)))
        ...
        it("Should have updated CDP getLvUSDBorrowed with values for leveraged position", async function () {
            expect(await r.cDPosition.getLvUSDBorrowed(endToEndTestNFTId)).to.equal(leverageToGetForPosition);
        });

        it("Should have updated CDP getShares with values for leveraged position", async function () {
            expect(getFloatFromBigNum(await r.cDPosition.getShares(endToEndTestNFTId)))
                .to.closeTo(getFloatFromBigNum(collateralAmount.add(depositedLeveragedOUSD)), 10);
        });
        ...
        it("Should have deposited principle plus leveraged OUSD into Vault minus origination fees", async function () {
            expect(getFloatFromBigNum(await r.vaultOUSD.totalAssets()))
                .to.closeTo(getFloatFromBigNum(collateralAmount.add(depositedLeveragedOUSD)), 10);
        });

        it("Should have transferred lvUSD out of coordinator minted amount", async function () {
            expect(await r.lvUSDToken.balanceOf(r.coordinator.address)).to.equal(mintedLvUSDAmount.sub(leverageToGetForPosition));
        });
    });
});
