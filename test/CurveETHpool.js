...
describe("Test Pools Rewards", () => {
    ...
    describe("CurveFi Test", () => {
        it("CurveFi Test ETH pool", async () => {
            ...
            let poolAddress = await curveFactory[
                "find_pool_for_coins(address,address)"
            ](weth.address, tUSD.address);
            ...
            let balanceSend = noExp(10 ** dectUSD * 100);
            ...
            await tUSD.approve(pool.address, noExp(10 ** 50));
            await weth.approve(pool.address, noExp(10 ** 50));
            ...
            await tUSD.connect(walletA).approve(pool.address, noExp(10 ** 50));
            //console.log(util.inspect(pool))
            await pool
                .connect(walletA)
                .functions["exchange(int128,int128,uint256,uint256)"](
                    1,
                    0,
                    noExp(balanceSend),
                    0
