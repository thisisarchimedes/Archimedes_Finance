...
describe("VaultOUSD test suit", function () {
    ...
    async function setupAndResetState () {
        ...
        await r.externalOUSD.connect(r.addr1).approve(r.vaultOUSD.address, getDecimal(addr1Deposit));
        await r.vaultOUSD.connect(r.addr1).archimedesDeposit(getDecimal(addr1Deposit), sharesOwnerAddress);
        await r.externalOUSD.connect(r.addr2).approve(r.vaultOUSD.address, getDecimal(addr2Deposit));
        await r.vaultOUSD.connect(r.addr2).archimedesDeposit(getDecimal(addr2Deposit), sharesOwnerAddress);
    }
    ...
    describe("Addr1 and addr2 signer deposited OUSD into vault", function () {
        ...
        it("Should return OUSD to be Vault's asset", async function () {
            const vaultAsset = await r.vaultOUSD.asset();
            expect(vaultAsset).to.equal(addressOUSD);
        });

        it("Should have an updated total assets sum after deposit", async function () {
            expect(await r.vaultOUSD.totalAssets()).to.equal(getDecimal(addr1Deposit + addr2Deposit));
        });

        it("Should have all shares under owners address", async function () {
            expect(await r.vaultOUSD.maxRedeem(sharesOwnerAddress)).to.equal(getDecimal(addr1Deposit + addr2Deposit));
        });
        ...
        describe("Adding more money to vault as interest (ie no shares are minted)", function () {
            before(async function () {
                ...
                await r.externalOUSD.transfer(r.vaultOUSD.address, getDecimal(interestIntoVault));
                expect(await r.externalOUSD.balanceOf(r.vaultOUSD.address)).to.equal(
            ...
            it("Should not change number of shares per deposit", async function () {
                ...
                expect(await r.vaultOUSD.maxRedeem(sharesOwnerAddress)).to.equal(
            ...
            it("Should redeem with each share worth more then 1 underlying", async function () {
                ...
                expect(await r.vaultOUSD.previewRedeem(getDecimal(addr1Deposit + addr2Deposit))).to.equal(
                    getDecimal(addr1Deposit + addr2Deposit + interestIntoVault),
                );
            });
        });
    ...
    describe("Test collecting rebase fees", function () {
        ...
        it("Should transfer fee to treasury on rebase event", async function () {
            await r.externalOUSD.connect(r.addr3).transfer(r.vaultOUSD.address, ethers.utils.parseUnits("10.0"));
        ...
        it("Should have previous assets in vault plus rebase minus fee", async function () {
            const assetsInVaultAfterRebase = getFloatFromBigNum(await r.vaultOUSD.totalAssets());
        ...
        it("Should NOT transfer funds to treasury on a redeem", async function () {
            await r.vaultOUSD.archimedesRedeem(getDecimal(addr1Deposit / 2), r.addr1.address, sharesOwnerAddress);
            const treasuryBalanceInNatural = getFloatFromBigNum(await r.externalOUSD.balanceOf(r.treasurySigner.address));
            expect(treasuryBalanceInNatural).to.equal(1);
        });
        it("Should transfer fee to treasury on (another) rebase event", async function () {
            await r.externalOUSD.connect(r.addr3).transfer(r.vaultOUSD.address, ethers.utils.parseUnits("20.0"));
            await r.vaultOUSD.takeRebaseFees();
            const treasuryBalanceInNatural = getFloatFromBigNum(await r.externalOUSD.balanceOf(r.treasurySigner.address));
