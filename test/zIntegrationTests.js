...
const { CURVE_FACTORY_ADDRESS, CURVE_FACTORY_ABI, CURVE_3POOL_ADDRESS, USDT_ADDRESS, abiCurveFactory } = require('./MainnetHelper');
...
describe("Setting the stage: Getting some OUSD and deploying our contracts", function () {
    ...
    it("Should do a basic ETH<>OUSD swap", async function () {
        ...
        await helper.helperSwapETHWithOUSD(user, ethers.utils.parseEther("1.0"))
    ...
    it("Should deploy lvUSD/3CRV pool", async function () {
        ...
        await curveFactory.deploy_metapool(helper.addressCurve3Pool, "lvUSD pool", "lvUSD", contractlvUSDToken.address, 10, 4000000)
