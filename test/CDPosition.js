...
describe("CDPosition test suit", async function () {
    ...
    const BASIC_OUSD_PRINCIPLE = getEighteenDecimal(
        BASIC_OUSD_PRINCIPLE_NATURAL
    );
    ...
    describe("Create position", () => {
        ...
        it("Should not create position on existing NFT ID", async function () {
            ...
            await expect(
                cdp.createPosition(NFT_ID, BASIC_OUSD_PRINCIPLE)
    ...
    describe("Delete Position", function () {
        ...
        it("Should not delete position if position still has borrowed lvUSD", async function () {
            ...
            await expect(cdp.deletePosition(NFT_ID)).to.be.revertedWith(
    ...
    describe("Borrow and deposit actions for position", function () {
        ...
        it("Should not mark down repayed lvUSD if not enough borrowed lvUSD", async function () {
            await expect(
                cdp.repayLvUSDToPosition(NFT_ID, getEighteenDecimal(1000))
            ).to.be.revertedWith(
        ...
        it("Should mark up deposited OUSD in NFT position", async function () {
            ...
            await validateCDP(
                ...
                0,
                getEighteenDecimal(
                    OUSD_AMOUNT_NATURAL + BASIC_OUSD_PRINCIPLE_NATURAL
                ),
                0,
                true
            );
        ...
        it("Should mark down OUSD withdrawn from position", async function () {
            ...
            await cdp.withdrawOUSDFromPosition(
                NFT_ID,
                getEighteenDecimal(30000)
            );
            await validateCDP(
                NFT_ID,
                BASIC_OUSD_PRINCIPLE,
        ...
        it("Should mark down OUSD withdrawn from position taking into account OUSD deposited after principle ", async function () {
            ...
            await cdp.depositOUSDtoPosition(NFT_ID, OUSD_AMOUNT);
            let OUSDBalanceInTotalAfterDepositNatural =
                BASIC_OUSD_PRINCIPLE_NATURAL + OUSD_AMOUNT_NATURAL;
            await validateCDP(
                NFT_ID,
                BASIC_OUSD_PRINCIPLE,
                0,
            ...
            await validateCDP(
                ...
                0,
                getEighteenDecimal(
                    OUSDBalanceInTotalAfterDepositNatural - 30000
                ),
                0,
                true
            );
        ...
        it("Should not mark down withdraw OUSD if total deposited OUSD is lower then amount to withdraw", async function () {
            await expect(
                cdp.withdrawOUSDFromPosition(
                    NFT_ID,
                    getEighteenDecimal(1100000)
                )
            ).to.be.revertedWith(
        ...
        it("Shouldn't allow to borrow more lvUSD if we above collateral rate", async function () {
            ...
            await expect(
                cdp.borrowLvUSDFromPosition(NFT_ID, totalOUSD + OUSD_AMOUNT)
    ...
    describe("Make sure that changes to a specific NFT ID CDP struct does not effect other NFT IDs struct", function () {
        ...
        let nftIDSecondaryPrinciple = getEighteenDecimal(
            BASIC_OUSD_PRINCIPLE_NATURAL * 2
        );
        beforeEach(async function () {
        ...
        let nftIDMainExpectedOUSDTotal = getEighteenDecimal(
            BASIC_OUSD_PRINCIPLE_NATURAL + 1500
        );
        ...
        it("Should not alter any NFT ID CDP if it tried to alter a non existing NFT", async function () {
            await expect(
                cdp.depositOUSDtoPosition(2349201840, amountInOUSDToDeposit)
            ).to.be.revertedWith("NFT ID must exist");
