...
export async function deployOrGetAllContracts(contracts: Contracts, deployJustTokens = false, deployArchimedesEngine = false, deployVault = false) {
    if (deployJustTokens) {
        ...
        Logger.log("Getting already deployed tokens instances from address");
        await contracts.setTokensInstances(DeployedStore.lvUSDAddress, DeployedStore.archTokenAddress);
        await contracts.setExternalTokensInstances();
    }

    if (deployArchimedesEngine) {
        Logger.log("Deploying most of Archimedes Engine(vault not included)");
        await contracts.initArchimedesUpgradableContracts();
    } else {
        Logger.log("Getting ArchimedesEngine deployed tokens instances from address(vault not included)");
    ...
    if (deployVault) {
        ...
        Logger.log("Getting Vault deployed token instances from address");
